...
const config = {
  libraryName: '@angular-material-extensions/layout',
  unscopedLibraryName: 'layout',
  allSrc: 'src/**/*',
  allTs: 'src/**/!(*.spec).ts',
...
gulp.task('build:demo-prerender', () => {
  return execDemoCmd(`build --preserve-symlinks --prod --base-href /layout/ --deploy-url /layout/`, {cwd: `${config.demoDir}`})
    .then(() => execDemoCmd(`run @angular-material-extensions/layout-demo:server:production`, {cwd: `${config.demoDir}`}))
